package com.dajukeji.hslz.view.flowlayout;
...
public class TagFlowLayout extends FlowLayout
    ...
    private GoodDetailsBean.ContentBean m;
    ...
    public void setAdapter(TagAdapter adapter,GoodDetailsBean.ContentBean m) {
        mTagAdapter = adapter;
        mTagAdapter.setOnDataChangedListener(this);
        this.m = m;

        if (this.m.getGoodsProductInfo().isSellProductTicket()){
    ...
    private void changeAdapter() {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            if (adapter.getCount() ==1){//当只有一个选项时，自动选中
                setChildChecked(i,tagViewContainer);
            }
    ...
    private void doSelect(TagView child, int position) {
        ...
        if (mOnSelectListener != null) {//如果接口不为空
