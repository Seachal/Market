 ...
 * 通过intent来传入URL信息
 */
class WebActivity : LkBaseActivity() {

	override fun layoutId() = R.layout.activity_web_test//设置布局，一个webView

	private val url: String by lazy { intent.getStringExtra("URL") }//解析出URL

	override fun onCreate(savedInstanceState: Bundle?) {
		super.onCreate(savedInstanceState)

		var webView = findViewById<WebView>(R.id.webView)
//		initWebView(webView, "http://dajukeji.com/exchangeArea/index.html#/activationquota")//测试用
	...
	private fun initWebView(webView: WebView, url: String) {
		...
		webView.addJavascriptInterface(DJJsBaseInterface(this), "DJJsBaseInterface")
		webView.addJavascriptInterface(DJJsPayInterface(this, webView), "DJJsPayInterface")
		webView.addJavascriptInterface(DJExchangeGoods(this),"DJExchangeGoods")
...
//			webView.loadUrl("javascript:onFrameViewBackPressed()")
//			return true
//		} else
	 ...
	 * 从兑换专区 跳到原生商品详情
	 */
	class DJExchangeGoods(val activity: WebActivity){
...
//			intent.putExtra("goods_id",goods_id.toInt())
	...
	/**供JS基础交互*/
	class DJJsBaseInterface(val activity: WebActivity) {
