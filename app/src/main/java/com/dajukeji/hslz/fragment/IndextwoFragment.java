...
import com.dajukeji.hslz.activity.TransactionDetails;
...
public class IndextwoFragment extends HttpBaseFragment {
    ...
    @Nullable//TODO FragmentAdapter 是不是不走 onCreateView 而走 onStart 导致原有数据丢失
    @Override
...
//        xRecyclerView.setLoadingMoreEnabled(false);
        ...
        recyclerAdapter.setOnItemClickListener(new BaseRecyclerAdapter.OnItemClickListener<CouponBean.DataBean.TransactionlistBean.ItemsBean>() {
            @Override
            public void onItemClick(BaseRecyclerHolder viewHolder, CouponBean.DataBean.TransactionlistBean.ItemsBean data, int position) {
                Intent intent = new Intent(getContext(),TransactionDetails.class);
                intent.putExtra("requestNo",data.getRequestNo());
                intent.putExtra("type",data.getBusinessType());
                intent.putExtra("shouzhi",data.getPayAmount());
                intent.putExtra("time",data.getPayTime());
